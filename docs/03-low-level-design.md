...
src/
  ...
  view/
    activityViewProvider.ts WebviewViewProvider 実装
    aggregation.ts          集計ロジック・日付按分
  util/
    time.ts                 ISO8601(UTC) 変換・日付範囲算出
media/
  main.js / main.css / icon.svg   WebView アセット (既存 icon.svg)
```
...
- `resolveWebviewView`: `webview.options = { enableScripts: true, localResourceRoots: [media] }`。`getHtml()` で CSP + nonce 付き HTML を生成し `media/main.js` / `media/main.css` を読み込む。`onDidReceiveMessage` を配線。
